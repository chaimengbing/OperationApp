...
public class EditTextInfoWindow extends BaseBottomWindow implements OnClickListener {
    ...
    public void initData() {//必须调用
        ...
        switch (intentType) {
            ...
            case TYPE_PASSWORD:
                maxEms = 15;
                etEditTextInfo.setInputType(InputType.TYPE_NUMBER_VARIATION_PASSWORD);
                break;
            default:
                break;
        }
    ...
    public void onForwardClick(View v) {
        editedValue =
                StringUtil.getTrimedString(tvEditTextInfoPlace) + StringUtil.getTrimedString(etEditTextInfo);
        if (editedValue.equals("" + getIntent().getStringExtra(INTENT_VALUE))) {
            CommonUtil.showShortToast(context, "内容没有改变哦~");
            return;
        }
        if (intentType == TYPE_PASSWORD) {
            if (StringUtil.isEmpty(editedValue)) {
                showShortToast(R.string.input_password);
                return;
            } else if (!ContactUtil.PASSWORD.equals(editedValue)) {
                showShortToast(R.string.password_is_incorrect);
                return;
            }
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case REQUEST_TO_PLACE_PICKER:
                List<String> list = data == null ? null :
