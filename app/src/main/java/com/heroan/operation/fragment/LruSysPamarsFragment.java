...
public class LruSysPamarsFragment extends BaseFragment implements View.OnClickListener,
...
//    private Button airSpeedButton;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        setContentView(R.layout.fragment_setting_lru_system);
        initView();
        initData();
        initEvent();
        return view;
...
//        airSpeedButton = (Button) view.findViewById(R.id.air_speed_setting_button);
    ...
    private void setData(String result) {
        String data;
        int value;
        if (result.contains(ConfigParams.Channel.trim())) {// 物理地址
            data = result.replaceAll(ConfigParams.Channel, "");
            macAdd.setText(data);
        } else if (result.contains(ConfigParams.PulseTime.trim())) {// 待机时间间隔
            data = result.replaceAll(ConfigParams.PulseTime, "");
            standBy.setText(data);
        } else if (result.contains(ConfigParams.PulsePolar.trim())) {// 卡配置
        ...
        } else if (result.contains(ConfigParams.SensorPolar.trim())) {// 卡配置
