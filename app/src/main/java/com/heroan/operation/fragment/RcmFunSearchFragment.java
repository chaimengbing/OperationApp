...
//    private String Collection_elements = "采集要素：";
    ...
    private void readData(String result, String content) {
        ...
        if (result.contains(ConfigParams.SetAddr)) {
            ...
            currentSB.insert(currentSB.indexOf(workMode) + workMode.length(), data.trim().equals(
                    "1") ? getString(R.string.always_online) : getString(R.string.low_power));
            ...
            currentSB.insert(currentSB.indexOf(timeInterval) + timeInterval.length(),
...
//            currentSB.insert(currentSB.indexOf(Collection_elements) + Collection_elements
// .length(),data.trim());
//        }
