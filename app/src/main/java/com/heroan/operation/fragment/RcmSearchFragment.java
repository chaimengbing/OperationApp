...
public class RcmSearchFragment extends BaseFragment implements EventNotifyHelper.NotificationCenterDelegate {
    ...
    private void readData(String result, String content) {
        ...
        if (result.contains(ConfigParams.SetIPChannel)) {
            ...
            currentSB.insert(currentSB.indexOf(netWorkStatus) + netWorkStatus.length(),
                    data.trim().equals("1") ? getString(R.string.Not_inserted) :
                            getString(R.string.normal));
        } else if (result.contains(ConfigParams.TFCardStatus)) {
            data = result.replaceAll(ConfigParams.TFCardStatus, "").trim();
            currentSB.insert(currentSB.indexOf(TFStatus) + TFStatus.length(), data.trim().equals(
                    "1") ? getString(R.string.Not_inserted) : getString(R.string.normal));
        } else if (result.contains(ConfigParams.SetSIMConfig)) {
            data = result.replaceAll(ConfigParams.SetSIMConfig, "").trim();
            currentSB.insert(currentSB.indexOf(simConfig) + simConfig.length(),
                    data.trim().equals("1") ? getString(R.string.China_Mobile) :
                            getString(R.string.China_Telecom));
            ...
            currentSB.insert(currentSB.indexOf(takePhotoInterval) + takePhotoInterval.length(),
                    ServiceUtils.getFunTimePos(time));
        } else if (result.contains(ConfigParams.SetVideoSave)) {
            data = result.replaceAll(ConfigParams.SetVideoSave, "").trim();
            int i = Integer.parseInt(data);
            currentSB.insert(currentSB.indexOf(videoTF) + videoTF.length(),
                    ServiceUtils.getFunTFPos(i));
        }
